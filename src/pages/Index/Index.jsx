...
function Index() {
  ...
  const currentCategory = searchParams.get("category"); // Получил ID текущей категории
  const currentPage = Number(searchParams.get("page")); // Получил номер текущей страницы
  ...
  useEffect(() => {
    if (filteredCollections.length === 0) { // Если объект с обработанными коллекциями ещё не создан
      ...
      setCurrentCollections(collectionsArrayNew[currentCategory]); // Текущая коллекция = коллекция текущей категории (по дефолту - "Все")
    } else { // Иначе берём готовый объект
      setCurrentCollections(filteredCollections[currentCategory]);
    }

  }, [unfilteredCollections, filteredCollections, currentCategory]);
    ...
    return (
      <AnimationPage>
        ...
        <CategoriesContext.Provider value={{
          ...
          setSearchedCollections, inputValue, setInputValue, currentCategory, currentPage, setSearchParams
        }}>
          <Header />
        </CategoriesContext.Provider>

        <CollectionContext.Provider value={{ currentPage, currentCategory, setSearchParams }}>
